...
import { addDays, minutesToTime, nowMinutes, toISODate, todayISO, weekDays } from '../../Support/date.js';
...
watch(
    ...
    (list) => {
        if (detailsAppointment.value) {
            detailsAppointment.value = list.find((item) => item.id === detailsAppointment.value.id) ?? null;
        }
    },
);
...
const weekEmployeeId = computed(() => props.selectedEmployeeId ?? props.employees[0]?.id ?? null);
const weekEmployee = computed(() => props.employees.find((employee) => employee.id === weekEmployeeId.value) || null);
...
const columns = computed(() => {
    ...
    const employees = props.selectedEmployeeId
        ? props.employees.filter((employee) => employee.id === props.selectedEmployeeId)
        : props.employees;
...
const gridAppointments = computed(() => {
    ...
    return props.appointments
        .filter((appointment) => !props.selectedEmployeeId || appointment.employee_id === props.selectedEmployeeId)
        .map((appointment) => ({ ...appointment, columnKey: String(appointment.employee_id) }));
});

const nowLine = computed(() => {
    if (props.view === 'week') {
        const inRange = props.today >= props.rangeStart && props.today <= props.rangeEnd;

        return inRange ? { minutes: nowMinutes(), columnKey: props.today } : { minutes: null, columnKey: null };
    }

    return props.date === props.today ? { minutes: nowMinutes(), columnKey: null } : { minutes: null, columnKey: null };
});
...
function reload(overrides = {}) {
    const params = {
        view: props.view,
        date: props.date,
        ...(props.selectedEmployeeId ? { employee_id: props.selectedEmployeeId } : {}),
        ...overrides,
    };
...
function defaultStart() {
    return props.date === props.today
        ? minutesToTime(Math.min(Math.max(nowMinutes(), props.settings.start_hour * 60), (props.settings.end_hour - 1) * 60))
        : minutesToTime(props.settings.start_hour * 60);
}
...
function onSlotClick({ columnKey, startMinutes }) {
    ...
    if (props.view === 'week') {
        formState.initial = buildCreateInitial({ employeeId: weekEmployeeId.value, date: columnKey, startTime });
    } else {
        formState.initial = buildCreateInitial({ employeeId: Number(columnKey), date: props.date, startTime });
    }
...
const confirmMoveMessage = computed(() => {
    ...
    const customer = confirmMove.value.appointment.customer_name ?? 'este agendamento';
    const target = confirmMove.value.targetEmployee?.name ?? 'outro profissional';

    return `Mover o agendamento de ${customer} para ${target}?`;
});

function commitReschedule({ id, employeeId, date, startMinutes }) {
    const startTime = minutesToTime(startMinutes);
    ...
    router.patch(
        ...
        { employee_id: employeeId, date, start_time: startTime },
        {
            preserveState: true,
            preserveScroll: true,
            optimistic: (current) => ({
...
function onReschedule({ id, columnKey, startMinutes }) {
    ...
    const employeeId = props.view === 'week' ? weekEmployeeId.value : Number(columnKey);
    ...
    if (props.view !== 'week' && employeeId !== appointment.employee_id) {
        confirmMove.value = {
            ...
            startMinutes,
            appointment,
            targetEmployee: props.employees.find((item) => item.id === employeeId) || null,
        };
...
<template>
    <AppLayout>
        ...
        <div class="space-y-4">
            ...
            <div v-if="view === 'week' && weekEmployee" class="flex items-center gap-2 text-sm text-secondary">
                <span class="size-2.5 rounded-full" :style="{ backgroundColor: weekEmployee.color }" />
                Exibindo a semana de <span class="font-medium text-foreground">{{ weekEmployee.name }}</span>
            </div>
